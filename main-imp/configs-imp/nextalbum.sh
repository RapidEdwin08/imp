...
if [ "$1" == '' ] && [ $(cat $IMPSettings/infinite.flag) == "1" ]; then
	# Shuffle the Current Playlist
	sort --random-sort $IMPPlaylist/init > $IMPPlaylist/shuffle
	if [ "$httpSTREAM" == '0' ]; then bash "$IMP/play.sh" & exit 0; fi
	nextTRACK=$(head -qn1 $currentPLIST)
	#nextTRACK=$(head -qn2 $currentPLIST | sed -n 2p)
	# Resume Playback
	bash "$IMP/rom.sh" "$nextTRACK" & exit 0
fi

# Respect Randomizer if Enabled
if [ $(cat $IMPSettings/randomizerboot.flag) == "1" ]; then bash "$IMP/randomizer.sh" & exit 0; fi
...
if [ $(cat $IMPSettings/a-side.flag) == "1" ] && [ $(cat $IMPSettings/b-side.flag) == "1" ]; then
	...
	if [[ ! "$dirBASE" == "$musicDIR/" ]]; then find "$musicDIR" -maxdepth 1 -type f -iname "*.mp3"  | grep -vi .m3u | grep -vi .pls | grep -v 'imp/music/bgm/startup.mp3' | grep -v 'imp/music/bgm/quit.mp3' | grep -i .mp3 > /dev/shm/init; fi
...
if [ $(cat $IMPSettings/a-side.flag) == "1" ] && [ $(cat $IMPSettings/b-side.flag) == "0" ]; then
	...
	if [[ ! "$dirBASE" == "$musicDIR/" ]]; then find "$musicDIR" -maxdepth 1 -type f -iname "*.mp3" | grep -vi .m3u | grep -vi .pls | grep -v "/bgm/B-SIDE/" | grep -v 'imp/music/bgm/startup.mp3' | grep -v 'imp/music/bgm/quit.mp3' | grep -i .mp3 > /dev/shm/init; fi
...
if [ $(cat $IMPSettings/a-side.flag) == "0" ] && [ $(cat $IMPSettings/b-side.flag) == "1" ]; then
	...
	if [[ ! "$dirBASE" == "$musicDIR/" ]]; then find "$musicDIR" -maxdepth 1 -type f -iname "*.mp3" | grep -vi .m3u | grep -vi .pls | grep -v "/bgm/A-SIDE/" | grep -v 'imp/music/bgm/startup.mp3' | grep -v 'imp/music/bgm/quit.mp3' | grep -i .mp3 > /dev/shm/init; fi
...
if [ $(cat $IMPSettings/a-side.flag) == "0" ] && [ $(cat $IMPSettings/b-side.flag) == "0" ]; then
	...
	if [[ ! "$dirBASE" == "$musicDIR/" ]]; then find "$musicDIR" -maxdepth 1 -type f -iname "*.mp3" | grep -vi .m3u | grep -vi .pls | grep -v "/bgm/A-SIDE/" | grep -v "/bgm/B-SIDE/" | grep -v 'imp/music/bgm/startup.mp3' | grep -v 'imp/music/bgm/quit.mp3' | grep -i .mp3 > /dev/shm/init; fi
...
if [ "$httpSTREAM" == '0' ]; then
	...
	grep -FA $LINEcount "$mp3BASE" $tmpPLIST | grep -v "$mp3BASE" | grep -v "$dirBASE" > /dev/shm/init
	grep -FB $LINEcount "$mp3BASE" $tmpPLIST | grep -v "$mp3BASE" | grep -v "$dirBASE" >> /dev/shm/init
fi
...
if [ "$httpSTREAM" == '0' ]; then
	...
	if [[ "$nextTRACK" == "$mp3BASE" ]]; then nextTRACK=$(head -qn2 /dev/shm/init | sed -n 2p); fi
else
	# Get Folder Folder from Last Line - Get nextTRACK for MP3 from the 1st Line
	nextDIR=$(dirname "$(head -qn1 /dev/shm/init)")
