...
# Remove BGM A-SIDE MP3s from musicDIR SUB-directories According to BGM Settings
if [ ! "$dirBASE" == "$BGMdir/"* 2>/dev/null ] && [ "$(cat $IMPSettings/a-side.flag)" == '0' ]; then
	cat "$IMPPlaylist/shuffle" | grep -v 'imp/music/bgm/A-SIDE/' > $IMPPlaylist/init
	cat "$IMPPlaylist/init" > $IMPPlaylist/shuffle
fi
# Remove BGM B-SIDE MP3s from musicDIR SUB-directories According to BGM Settings
# Skip if currentTRACK is in BGM A-SIDE to Continue to B-SIDE
if [ ! "$dirBASE" == "$BGMdir/"* 2>/dev/null ] && [ "$(cat $IMPSettings/b-side.flag)" == '0' ]; then
	cat "$IMPPlaylist/shuffle" | grep -v 'imp/music/bgm/B-SIDE/' > $IMPPlaylist/init
	cat "$IMPPlaylist/init" > $IMPPlaylist/shuffle
fi
...
# Parse all lines after and before Last Track played again if nothing - Alternate FIlter
